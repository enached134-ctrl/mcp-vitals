...
from .l4_adversarial import LayerResult as L4Result
from .l5_ops import LayerResult as L5Result
...
def render(
    ...
    if l4 is not None:
        layer_scores["L4"] = l4.score
    if l5 is not None:
        layer_scores["L5"] = l5.score
    ...
    if l4 is not None:
        if l4.findings:
            items = "".join(
                f'<div class="l4row"><span class="l4sev">-{f.severity}</span>'
                f'<span class="l4w">{f.where}</span><span class="l4l">{f.label}</span></div>'
                for f in l4.findings
            )
        else:
            items = ('<div class="l4clean">✓ No tool-poisoning or over-permission patterns '
                     'detected in descriptions, prompts, or resources.</div>')
        l4_section = f"""<h2>L4 · Adversarial — tool-poisoning &amp; over-permission scan</h2>
          <p style="color:#7C89A0;font-size:14px;margin:-6px 0 14px;font-family:'JetBrains Mono',monospace">
          score {l4.score:.0f}/100 · {l4.summary} · <em>heuristics, no server calls</em></p>{items}"""
    ...
    if l5 is not None:
        ...
        l5_section = f"""<h2>L5 · Ops — transport &amp; operational hygiene</h2>
          <p style="color:#7C89A0;font-size:14px;margin:-6px 0 14px;font-family:'JetBrains Mono',monospace">
          score {l5.score:.0f}/100 · {l5.summary}</p><ul class="checks">{items}</ul>"""
...
.l4row{{display:flex;align-items:center;gap:16px;background:rgba(45,20,20,.4);border-radius:10px;padding:12px 18px;margin-bottom:8px;border-left:3px solid #F87171}}
.l4sev{{font-family:'JetBrains Mono',monospace;font-weight:700;color:#FCA5A5;min-width:44px}}
.l4w{{font-family:'JetBrains Mono',monospace;font-size:14px;color:#94A3B8;min-width:180px}}
.l4l{{color:#E8EEF6;font-size:15.5px}}
.l4clean{{background:rgba(6,50,44,.3);border:1px solid rgba(52,211,153,.35);border-radius:10px;padding:16px 20px;color:#8FE9CE;font-size:16px}}
...
<div class="foot">Generated by mcp-vitals · all five layers: L1 static · L2 behavioral · L3 agent-usability · L4 adversarial · L5 ops.</div>
</div></body></html>"""
